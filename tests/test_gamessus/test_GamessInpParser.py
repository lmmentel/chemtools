...
class TestGIPonHeH2(unittest.TestCase):
    ...
    def test_contrl(self):
        d ={"scftyp" : "rhf",
            ...
            "ispher" : "1",
            "units" : "bohr"}
        self.assertDictEqual(d, self.inpd["$contrl"])

    def test_trans(self):
        d = {"cuttrf" : "1.0d-10"}
        self.assertDictEqual(d, self.inpd["$trans"])

    def test_system(self):
        d = {"timlim" : "525600",
             "mwords" : "100"}
        self.assertDictEqual(d, self.inpd["$system"])

    def test_title(self):
        self.assertEqual(self.inpd["$data"]["title"], "He-H2 FCI")

    def test_group(self):
        self.assertEqual(self.inpd["$data"]["group"], "cnv 2")

    def test_atoms(self):
        self.assertEqual(len(self.inpd["$data"]["atoms"]), 2)

    def  test_atom1_xyz(self):
        t = tuple([0.0, 0.0, -3.0])
        self.assertTupleEqual(t, self.inpd["$data"]["atoms"][0]["xyz"])

    def  test_atom1_atomic(self):
        self.assertEqual(self.inpd["$data"]["atoms"][0]["atomic"], "2.0")

    def  test_atom1_label(self):
        self.assertEqual(self.inpd["$data"]["atoms"][0]["label"], "He")

    def  test_atom2_xyz(self):
        t = tuple([0.0, 0.724368, 3.0])
        self.assertTupleEqual(t, self.inpd["$data"]["atoms"][1]["xyz"])

    def  test_atom2_atomic(self):
        self.assertEqual(self.inpd["$data"]["atoms"][1]["atomic"], "1.0")

    def  test_atom2_label(self):
        self.assertEqual(self.inpd["$data"]["atoms"][1]["label"], "H")
...
14  3 0.00000000E+00 0.00000000E+00 0.00000000E+00 0.00000000E+00 0.00000000E+00
 $END'''
    ...
    def setUp(self):
        self.gip = GamessInpParser()
        self.inpd = self.gip.parse_from_string(TestGIPonNe.ne_no_inp)

    def test_inpdict(self):
        self.assertIsInstance(self.inpd, dict)

    def test_vec(self):
        self.assertIsInstance(self.inpd["$vec"], str)
