...
class TestSequentialFileHeMini(unittest.TestCase):
    ...
    def setUp(self):
        seqfile = os.path.join(os.path.abspath(os.path.dirname(__file__)),
                               'data/he/he_mini_hf.F08')
...
class TestSequentialFileNeDZAO(unittest.TestCase):
    ...
    def test_read_ao_integrals_ne(self):
        ints = np.load(os.path.join(os.path.abspath(os.path.dirname(__file__)),
                                    'data/ne/ne_dz_guga_aoints.npy'))
        self.assertTrue(np.allclose(self.seq.readseq(), ints))
